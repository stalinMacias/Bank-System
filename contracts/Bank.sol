...
contract BANK {
    ...
    modifier enoughFunds(uint _amount) {
        require(depositBalance[msg.sender] - _amount >= 0, "Reverting transaction, user has not enough funds to perform this action, deposit more funds to increase the user's balance");
        _;
    }
    ...
    */
    ...
    function payLoan(uint _amount) public enoughFunds(_amount) {
        require(activeLoan[msg.sender] == true, "Reverting transaction, the user has not an active loan.");
        // Take the tokens that the user is paying from the depositBalance
        depositBalance[msg.sender] = depositBalance[msg.sender].sub(_amount);
        // Update userDebt accordingly to the amount that the user is paying
        userDebt[msg.sender] = userDebt[msg.sender].sub(_amount);
        if(userDebt[msg.sender] == 0) {
            // The user has paid its debt!!!
            activeLoan[msg.sender] = false;
        }
    }
